...
def detect_input_kind(path: str | Path) -> str:
    ...
    if suffix == ".csv":
        # A CSV may be a hardware BOM (MPN/manufacturer columns) or a plain
        # software component list.
        from ..inputs.hbom_reader import looks_like_hbom

        try:
            head = p.read_text(encoding="utf-8", errors="replace")[:2000]
        except OSError:
            head = ""
        return "hbom" if looks_like_hbom(head) else "manual"
    ...
    # Text file: hardware BOM, hardware probe, scanner table, or SBOM?
    try:
        head = p.read_text(encoding="utf-8", errors="replace")[:4000]
    except OSError:
        head = ""
    from ..inputs.hbom_reader import looks_like_hbom
    from ..inputs.hw_probe import looks_like_hwprobe
    from ..inputs.manual_entry import looks_like_table

    if looks_like_hbom(head):
        return "hbom"
    if looks_like_hwprobe(head):
        return "hwprobe"
...
def load_components(
    ...
    if kind in ("directory", "binary"):
        ...
        from ..inputs.hbom_reader import read_hbom

        comps = read_hbom(path)
    elif kind == "hwprobe":
        from ..inputs.hw_probe import read_hwprobe

        comps = read_hwprobe(path)
