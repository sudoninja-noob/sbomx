...
def build_bom_dict(components: List[Component]) -> Dict:
    ...
    for c in components:
        entry = {
            "type": c.type if c.type in ("application", "library", "firmware", "operating-system") else "library",
            "bom-ref": c.ref,
            "name": c.name,
            "version": c.version,
        }
...
def build_bom_xml(components: List[Component]) -> str:
    ...
    for c in components:
        ctype = c.type if c.type in ("application", "library", "firmware", "operating-system") else "library"
        comp_el = _el(comps_el, "component", type=ctype, **{"bom-ref": c.ref})
