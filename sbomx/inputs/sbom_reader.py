...
_CDX_TYPE_IN = {"device": "hardware", "operating-system": "os"}
...
def _parse_cyclonedx_json(data: dict) -> Tuple[List[Component], Dict]:
    ...
    for c in data.get("components", []):
        ...
        comp = Component(
            ...
            type=_CDX_TYPE_IN.get(ctype, ctype),
            vendor=c.get("publisher") or c.get("group")
            or (c.get("manufacturer") or {}).get("name"),
            purl=c.get("purl"),
            license=_cdx_license(c.get("licenses")),
            mpn=c.get("mpn"),
        )
        # MPN / reference designator may also arrive as properties.
        for prop in c.get("properties", []) or []:
            if prop.get("name") == "mpn" and not comp.mpn:
                comp.mpn = prop.get("value")
            elif prop.get("name") == "reference-designator":
                comp.part_ref = prop.get("value")
...
_SPDX_PURPOSE_IN = {
    "DEVICE": "hardware", "FIRMWARE": "firmware",
    "OPERATING-SYSTEM": "os", "APPLICATION": "application", "LIBRARY": "library",
}
